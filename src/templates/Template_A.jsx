              ...
              <Card className="border">
                ...
                <div>
                  {education
                    .slice()
                    .sort((a, b) => {
                      // tumia tarehe ya mwisho au ya kuanza kama backup
                      const bDate =
...
/** ---------- DATE HELPERS (fixes "YYYY-MM-DD HH:mm:ss") ---------- **/
function parseDate(d) {
  if (!d) return null;
  try {
    if (d instanceof Date) return d;
    if (typeof d === "string") {
      // chukua sehemu ya tarehe pekee kama ipo (YYYY-MM-DD)
      const datePart = d.split(/[ T]/)[0];
      if (/^\d{4}-\d{2}-\d{2}$/.test(datePart)) {
        // tengeneza ISO salama
