  ...
  useEffect(() => {
    if (candidate?.experience) {
      ...
      candidate.experience.forEach((item) => {
        if (
          !uniqueExperiences.some((e) => e.employer?.id === item.employer?.id)
        ) {
      ...
      <Row className="g-4 py-4">
        <Col md={4}>
            ...
            {candidate?.education?.length > 0 ? (
              candidate.education.map((edu, i) => (
                <div key={i} className="mb-3">
                  <h5 className="medium fw-bold" style={{ fontSize: "1rem" }}>
                    {edu?.course?.course_name}
                  </h5>
                  <p className="small mb-1">{edu?.level?.education_level}</p>
                  <p className="small mb-1">{edu?.college?.college_name}</p>
                  <p className="text-muted small">
                    {edu?.started ? new Date(edu.started).getFullYear() : ""} -{" "}
                    {edu?.ended ? new Date(edu.ended).getFullYear() : "Present"}
                  </p>
                </div>
              ))
            ) : (
          ...
          <Section title="SKILLS" titleStyle={{ fontSize: "0.9rem" }}>
            <SkillCategory
              title="Culture"
              items={candidate?.culture?.map((c) => c?.culture?.culture_name)}
            />
            <SkillCategory
              title="Software"
              items={candidate?.software?.map(
                (s) => s?.software?.software_name
              )}
            />
            <SkillCategory
              title="Tools"
              items={candidate?.tools?.map((t) => t?.tool?.tool_name)}
            />
            <SkillCategory
              title="Knowledge"
              items={candidate?.knowledge?.map(
                (k) => k?.knowledge?.knowledge_name
              )}
            />
          </Section>
        ...
        <Col md={8}>
                ...
                <div key={i} className="mb-4">
                  <div className="d-flex justify-content-between mb-2">
                    <h5 className="fw-bold">
                      {exp?.employer?.employer_name || "Employer"}
                    </h5>
                    <p className="text-muted small">
                      {exp?.start_date
                  ...
                  {exp?.positions?.map((pos, j) => (
                    ...
                    >
                      <h6 className="fw-bold text-primary">
                        {pos?.position?.position_name || "Position"}
                      </h6>
          ...
          <Section
            title="PROFICIENCY QUALIFICATIONS"
            titleStyle={{ fontSize: "0.9rem" }}
          >
            {candidate?.proficiency?.length > 0 ? (
              candidate.proficiency.map((prof, i) => (
                <div key={i} className="mb-3">
                  <div className="d-flex justify-content-between">
                    ...
                    <p className="text-muted small">
                      {moment(prof?.started).format("YYYY")} -{" "}
                      {prof?.ended
                        ? moment(prof.ended).format("YYYY")
                        : "Present"}
                    </p>
                  </div>
                  <p className="text-primary mb-1">
                    {prof?.organization?.organization_name}
                  </p>
                  <p className="small">
          ...
          >
            {candidate?.training?.length > 0 ? (
              candidate.training.map((train, i) => (
                <div key={i} className="mb-3">
                  <div className="d-flex justify-content-between">
                    <h5 className="fw-bold" style={{ fontSize: "1rem" }}>
                      {train?.name || "Training"}
                    ...
                    <p className="text-muted small">
                      {train?.started} - {train?.ended || "Present"}
                    </p>
                  </div>
            ...
            {candidate.referees.map((ref, i) => (
              <Col key={i} md={4}>
                <Card className="h-100">
                  <Card.Body>
                    <h5 className="fw-bold" style={{ fontSize: "1rem" }}>
                      {[ref?.first_name, ref?.middle_name, ref?.last_name]
                        .filter(Boolean)
                        .join(" ")}
                    </h5>
...
const Section = ({ title, children, titleStyle, titleClassName }) => (
  <div className="mb-4">
    <h4
      className={`bg-primary text-white py-2 px-3 fw-bold rounded-top ${
        titleClassName || ""
      }`}
      style={{
